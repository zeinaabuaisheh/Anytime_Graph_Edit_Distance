...
public class MatrixGenerator 
{
	...
	public double getCVE_DI(Graph sourceGraph, Graph targetGraph)
	{
		...
		{
			u = (Node) this.source.get(i);
			vedi +=  Constants.costFunction.getCosts(u,
					Constants.EPS_COMPONENT);
			for(int k=0;k<u.getEdges().size();k++)
			{
				Edge e = (Edge) u.getEdges().get(k);
				vedi += Constants.costFunction.getCosts(e,
						Constants.EPS_COMPONENT);
			}
		}
		...
		for(int i=0;i<target.size();i++)
		{
			...
			vedi +=  Constants.costFunction.getCosts(
					Constants.EPS_COMPONENT,v);
	...
	public float[][] getCQ1eMatrix(Graph sourceGraph, Graph targetGraph) 
	{
		...
		for (int i = 0; i < this.source.size(); i++) 
		{
			...
			for (int j = 0; j < this.target.size(); j++) 
			{
				...
				double insertcost=0;
				insertcost = Constants.costFunction.getCosts(
						Constants.EPS_COMPONENT, v);
				...
				double delcost = Constants.costFunction.getCosts(u,
						Constants.EPS_COMPONENT);
				for(int k=0;k<u.getEdges().size();k++)
				{
					Edge e = (Edge) u.getEdges().get(k);
					delcost += Constants.costFunction.getCosts(e,
							Constants.EPS_COMPONENT);
				}
